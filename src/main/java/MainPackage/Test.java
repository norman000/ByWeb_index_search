...
public class Test {
    public static void write_xml(ArrayList<ScoreDoc[]> list_doc_wich_rel, ArrayList<String> need_query_array) throws IOException, JDOMException {
        try {
            ...
            for (int i = 0; i < list_doc_wich_rel.size(); i++) {
                ...
                xmlOutput.output(doc, new FileWriter("/lucene_index_file/new_xml/" + name_queries + ".xml"));
    ...
    public static void main(String[] args) throws IOException, JDOMException, ParseException {
        ...
        ArrayList<String> need_query_array = XmlWork.get_list_query("/lucene_index_file/data/new/and_relevant-minus_table.xml");

        //номер запроса, номер документа - релевантность
        HashMap<String, HashMap> get_relevance_hashmap = XmlWork.get_relevance_hashmap("/lucene_index_file/data/and_relevant-minus_table.xml");

        System.out.println(get_relevance_hashmap.get("arw53809").size());

        //номер запроса - текст запроса
        HashMap<String, String> hashmap = XmlWork.get_id_query("/lucene_index_file/data/new/web2008_adhoc.xml");
